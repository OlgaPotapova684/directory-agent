...
|------------|-------------|
| `PORT` | На сервере (Render задаёт сам) |
| `PUBLIC_URL` | После деплоя: `https://ваш-сервис.onrender.com` — для учебного каталога и Playwright |
| `HOST` | По умолчанию `0.0.0.0` в Docker |

---

## Деплой (Render + Docker)

Проект можно выложить в интернет как **один сервис**: веб-интерфейс, учебный каталог и Playwright в одном контейнере.

### Ограничения в облаке

- **Память:** Playwright + Chromium требуют ~512 МБ и больше; на бесплатном тарифе возможны таймауты.
- **Данные:** без постоянного диска SQLite и логотипы сбрасываются при перезапуске (для демо обычно достаточно).
- **Парсинг чужих сайтов:** некоторые сайты блокируют IP дата-центров.

### Шаги на [Render](https://render.com)

1. Зарегистрируйтесь и подключите GitHub.
2. **New → Blueprint** → репозиторий `directory-agent` (в корне есть `render.yaml`).
3. Дождитесь сборки Docker-образа (5–10 минут первый раз).
4. В **Environment** добавьте переменную:
   - `PUBLIC_URL` = `https://<имя-сервиса>.onrender.com` (скопируйте URL из панели Render).
5. **Manual Deploy** → Redeploy, если сервис уже поднялся без `PUBLIC_URL`.
6. Откройте URL в браузере → **Использовать учебный каталог** → полный учебный цикл.

### Локально через Docker

```bash
docker build -t directory-agent .
docker run --rm -p 3000:3000 -e PUBLIC_URL=http://localhost:3000 directory-agent
```
